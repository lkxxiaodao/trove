...
class _SearchWorker(QRunnable):
    ...
    def __init__(self, keyword, clip_store, note_store, task_store, callback):
        super().__init__()
    ...
    def run(self):
        ...
        # TaskFlow
        task_results = self._task_store.search(keyword) if self._task_store else []
        results.append({"module": "task", "label": "定时任务", "results": task_results})
...
class SearchEngine(QObject):
    ...
    用法:
        engine = SearchEngine(clip_store, note_store, task_store)
    ...
    def __init__(self, clip_store=None, note_store=None, task_store=None):
        super().__init__()
        self._clip_store = clip_store
        self._note_store = note_store
        self._task_store = task_store
    ...
    def search(self, keyword: str):
        ...
        worker = _SearchWorker(
            keyword, self._clip_store, self._note_store, self._task_store,
            self._on_results,
        )
