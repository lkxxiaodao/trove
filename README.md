...
如果您不愿意配置环境，也可以去往我的爱发电主页，花一笔小钱即可永久获得软件的安装包与后续的持续更新。
爱发电地址：https://ifdian.net/a/lkxxd
...
- **图文混排**：支持插入本地图片（png / jpg / gif / bmp / webp），悬浮窗中图片随窗口缩放自适应
- **自定义配色**：8 种预设主题色，每种可单独配置背景色和字体颜色；支持取色器自由选择任意颜色
- **标签与颜色筛选**：单选标签（最多 5 字），支持按颜色 / 标签快速筛选
- **拖拽排序**：自定义排序模式下可手动拖拽卡片重新排列
- **悬浮笔记**：可将任意笔记「悬浮」到桌面，无边框置顶显示，自由移动和调整大小
- **锁 / 解锁**：悬浮窗左下角一键锁定，锁定后内容只读防误触；解锁可直接编辑
- **批量操作**：批量选择模式，一键删除 / 设置颜色 / 设置标签
- **全文搜索**：基于 SQLite FTS5 的实时全文检索
- **任务笔记**：勾选框清单模式，每行一个可勾选项，支持定时弹出和开机自启
- **回收站**：软删除机制，可恢复误删笔记或永久清空
- **普通↔任务互转**：两种笔记类型可随时切换

### ⏰ TaskFlow — 本地定时调度

- **5 种时间规则**：一次性、每天、每周（可指定星期几）、每月（指定日期 / 月末）、间隔 N 天
- **弹窗提醒**：支持图文混排的提醒内容，图片自动适配窗口大小
...
- **跨模块聚合**：同时搜索剪贴板历史、微笔记与定时任务三个模块
- **全局热键**：`Ctrl + Alt + V` 在任意窗口唤出浮动搜索弹窗，可拖拽移动
...
- **智能定位**：弹窗自动出现在当前光标附近，可拖拽标题栏移动位置
- **一键回填**：选中历史条目后自动模拟 Ctrl+V 粘贴到前台应用

### 🎨 界面主题

- 8 种 Material Design 主题（浅蓝/深蓝等中文名称），切换即刻生效
- 主题下拉框禁用滚轮防误触
...
│       ├── note_card.py            # NoteCard — 普通笔记卡片控件
│       ├── note_editor.py          # NoteEditor — 笔记编辑对话框（支持任务笔记/定时/图片）
│       ├── note_float.py           # NoteFloatWindow — 桌面悬浮笔记窗（普通笔记）
│       ├── task_note_card.py       # TaskNoteCard — 任务笔记勾选框卡片
│       ├── task_note_float.py      # TaskNoteFloat — 任务笔记交互式悬浮窗
│       ├── clip_item_delegate.py   # ClipItemDelegate — 剪贴板列表自定义绘制
│       ├── task_editor.py          # TaskEditor — 任务编辑对话框
│       └── task_reminder_popup.py  # TaskReminderPopup — 提醒弹窗（图文+可拖拽缩放）
│
└── data/                     # 数据访问层
...
| **NoteNest** | 自适应网格卡片墙，支持图文混排、自定义配色、拖拽排序、颜色 / 标签筛选、批量操作 |
